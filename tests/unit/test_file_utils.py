...
class TestParseInteractionFile:
    ...
    def test_parse_missing_model_id(self, tmp_path):
        ...
        test_file = tmp_path / "inter_0001.json"
        interaction_data = {"tokens": {"input": 1000, "output": 500}}
        test_file.write_text(json.dumps(interaction_data))
...
class TestLoadAllSessions:
    ...
    def test_load_multiple_sessions(self, tmp_path):
        ...
        inter1.write_text(
            json.dumps({"modelID": "model-a", "tokens": {"input": 1000, "output": 500}})
        )
        ...
        inter2.write_text(
            json.dumps(
                {"modelID": "model-b", "tokens": {"input": 2000, "output": 1000}}
            )
        )
    ...
    def test_load_sessions_with_limit(self, tmp_path):
        ...
        for i in range(5):
            ...
            inter.write_text(json.dumps({"tokens": {"input": 100, "output": 50}}))
...
class TestValidateSessionStructure:
    ...
    def test_valid_session_directory(self, tmp_path):
        ...
        (session_dir / "inter_0001.json").write_text(
            json.dumps({"tokens": {"input": 100, "output": 50}})
        )
...
class TestGetMostRecentSession:
    ...
    def test_get_most_recent(self, tmp_path):
        ...
        session1.mkdir()
        (session1 / "inter_0001.json").write_text(
            json.dumps({"tokens": {"input": 100}})
        )

        session2 = tmp_path / "ses_002"
        session2.mkdir()
        (session2 / "inter_0001.json").write_text(
            json.dumps({"tokens": {"input": 200}})
        )
